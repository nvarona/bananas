import tkinter as tk
from tkinter import PhotoImage, Canvas
import pyautogui
import random

class Banana:
    def __init__(self, scene, x=0, y=0):
        self.scene = scene
        self.image = PhotoImage(file='banana.png')
        self.image = self.image.subsample(16)
        self.image_bomb = PhotoImage(file='bomb.png')
        self.image_bomb = self.image_bomb.subsample(8)
        self.imageRef = scene.canvas.create_image(x, y, image=self.image)
        self.bomb_status = False

    def update(self):
        x, y = pyautogui.position()
        ban_x, ban_y = self.scene.canvas.coords(self.imageRef)
        dist = (abs(x-ban_x) + abs(y-ban_y))
        if self.bomb_status:
            self.scene.canvas.move(
                self.imageRef,
                random.choice((-30, 30)),
                random.choice((-30, 30))
            )
            self.scene.canvas.itemconfig(self.imageRef, image=self.image)
            for _ in range(5):
                self.scene.new_banana(
                    random.randint(0, self.scene.screen_width),
                    random.randint(0, self.scene.screen_height),
                )
            self.bomb_status = False
        elif dist < 5:
            self.scene.canvas.itemconfig(self.imageRef, image=self.image_bomb)
            self.bomb_status = True
        else:
            numero = random.choice((1,2,5))
            self.scene.canvas.move(
                self.imageRef,
                numero if x > ban_x else -numero,
                numero if y > ban_y else -numero
            )

class Scene:
    def __init__(self, window: tk.Tk):
        self.screen_width = window.winfo_screenwidth()
        self.screen_height = window.winfo_screenheight()
        self.canvas = Canvas(
            window,
            width=self.screen_width,
            height=self.screen_height,
            highlightthickness=0
        )
        self.canvas.pack()
        self.bananas = list()
    
        # Colocar un píxel transparente para establecer el fondo como transparente
        self.canvas.create_rectangle(0, 0, 1, 1, fill='white', outline='white')
    
    def update(self):
        for banana in self.bananas:
            banana.update()

    def new_banana(self, x, y):
        banana = Banana(self)
        self.canvas.move(banana.imageRef, x, y)
        self.bananas.append(banana)

class Game:
    def __init__(self):
        self.window = self.create_window()
        self.scene = Scene(self.window)
        self.scene.new_banana(100, 100)
        # Asociar la función self.on_closing con el evento de cierre de la ventana
        self.window.protocol("WM_DELETE_WINDOW", self.on_closing)
        # Maximizar la ventana en macOS
        self.maximize_macos()
    
    def maximize_macos(self):
        # Simular la tecla de maximizar en macOS utilizando pyautogui
        pyautogui.hotkey("ctrl", "cmd", "f")
        screen_width = self.window.winfo_screenwidth()
        screen_height = self.window.winfo_screenheight()

        # Establecer el tamaño de la ventana para que coincida con el tamaño de la pantalla
        self.window.geometry(f"{screen_width}x{screen_height}+0+0")

    def update(self):
        try:
            # Verificar si la ventana se ha destruido antes de actualizar la escena
            if not self.window.winfo_exists():
                return

            self.scene.update()
            self.window.after(15, self.update)
        except tk.TclError:
            # Capturar el error si se intenta acceder a la ventana después de ser destruida
            pass

    def create_window(self):
        window = tk.Tk()
        window.wm_attributes("-topmost", True)
        window.overrideredirect(True)
        # Configurar transparencia (compatibilidad con Linux y macOS)
        window.attributes('-alpha', 0.7)
        return window
    
    def on_closing(self):
        # Detener cualquier proceso o actualización pendiente
        # Puedes agregar aquí cualquier limpieza o detención necesaria antes de cerrar la ventana
        # ...
        # Esta función se ejecutará al intentar cerrar la ventana
        self.window.destroy()

    def start(self):
        self.update()
        self.window.mainloop()

game = Game()
game.start()
